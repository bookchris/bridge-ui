import { arrayUnion, doc, DocumentReference, getDoc, onSnapshot, QueryDocumentSnapshot, updateDoc } from 'firebase/firestore';
...
export function useTable(id: string): [Table | undefined, boolean, Error | undefined] {
    const [table, setTable] = useState<Table>();
    const [error, setError] = useState<Error>();
    useEffect(() => {
        const unsub = onSnapshot(tableDoc(id), (d) => {
...
export function useBid() {
    ...
    return useCallback(async (bid: string, seat: Seat) => {
        ...
        if (!table.bidding.validateNext(bid)) throw new Error(`Bid ${bid} is not valid`);
...
async function get(tableId: string): Promise<[DocumentReference, QueryDocumentSnapshot, Table]> {
    const ref = tableDoc(tableId);
