import { Seat, Suit, Suits } from "@chrisbook/bridge-core";
import {
  Alert,
  Box,
  Button,
  ...
  return (
    <Paper sx={{ m: 2, p: 2, width: "100%", maxWidth: 800 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "right", mt: 1 }}>
        <Button
          disabled={!input}
          onClick={() => performImport(input).then(onImport, setError)}
        >
          Import
        </Button>
      </Box>
...
const LinToSeat: { [bid: string]: Seat } = {
  "1": Seat.South,
  "2": Seat.West,
  "3": Seat.North,
  "4": Seat.East,
};
const LinToSuit: { [bid: string]: Suit } = {
  S: Suit.Spade,
  H: Suit.Heart,
  D: Suit.Diamond,
  C: Suit.Club,
