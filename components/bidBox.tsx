        ...
        <Paper sx={{ p: 1 }} >
            <Box display="flex">
                {Array.from({ length: 7 }, (_, i) => i + 1).map((l) => (
                    <BidButton key={l} variant={level === l ? "outlined" : "contained"} onClick={() => setLevel(l)} disabled={!!level || l < minLevel} sx={{ minWidth: 0 }}>{l}</BidButton>
                ))}
            </Box>
            {level ? (
                <div>
                    <Box display="flex">
                        <BidButton startIcon={<Icon>navigate_before</Icon>} onClick={() => setLevel(0)} />
                        {["♣", "♦", "♥", "♠", "NT"].map((suit) => {
...
function BidButton(props: ButtonProps) {
    return <Button variant="contained" fullWidth {...props} sx={{ minWidth: 0, m: 0.5, px: { xs: 1, sm: 2 } }} />
}