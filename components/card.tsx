...
export function Card2({ card, orientation, ...paperProps }: CardProps) {
    const Child = orientation == Orientation.Left ? LeftCard : orientation == Orientation.Right ? RightCard : VerticalCard;
    return (<Child card={card} {...paperProps} />);
}

export interface ChildProps extends PaperProps {
    card: number;
}

function VerticalCard({ card, ...paperProps }: ChildProps) {
    return (
        <Paper {...paperProps} sx={{ display: "inline-block", backgroundColor: "white", boxShadow: 1, width: "min(10vw, 80px)", height: "min(14vw, 112px)", ...paperProps?.sx }}>
            <CardImage src={IMAGES[card + 1]} width="100%" height="100%" />
        </Paper >
    )
}

function LeftCard({ card, ...paperProps }: ChildProps) {
    return (
        <Paper {...paperProps} sx={{ display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "white", boxShadow: 1, height: "min(10vw, 80px)", width: "min(14vw, 112px)", ...paperProps?.sx }}>
            <CardImage src={IMAGES[card + 1]} sx={{ width: "min(10vw, 80px)", height: "min(14vw, 112px)", transform: "rotate(270deg)" }} />
        </Paper >
    )
}
function RightCard({ card, ...paperProps }: ChildProps) {
    return (
        <Paper {...paperProps} sx={{ display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "white", boxShadow: 1, height: "min(10vw, 80px)", width: "min(14vw, 112px)", ...paperProps?.sx }}>
            <CardImage src={IMAGES[card + 1]} sx={{ width: "min(10vw, 80px)", height: "min(14vw, 112px)", transform: "rotate(90deg)" }} />
        </Paper >
    )
}
